...
// FIL *file_tmp;
 FIL *file_usb;
 FIL *ftemp;	 
 UINT br_sd,bw_sd,br_usb,bw_usb;
 FILINFO fileinfo_sd,fileinfo_usb;
 DIR dir_sd,dir_usb;
 u8 *fatbuf;//SD卡数据缓存区
 u8 *fatbuf_usb; //USB数据缓冲区
///////////////////////////////////////////////////////////////////////////////////////
//为exfuns申请内存
//返回值:0,成功
//1,失败
u8 exfuns_init(void)
{
	...
	file_sd=(FIL*)mymalloc(SRAMIN,sizeof(FIL));		//为file申请内存
	file_usb=(FIL*)mymalloc(SRAMIN,sizeof(FIL));	//为file申请内存
	ftemp=(FIL*)mymalloc(SRAMIN,sizeof(FIL));		  //为ftemp申请内存
	fatbuf=(u8*)mymalloc(SRAMIN,512);				      //为fatbuf申请内存
	fatbuf_usb=(u8*)mymalloc(SRAMIN,512);				  //为fatbuf申请内存
	if(i==_VOLUMES&&file_sd&&file_usb&&ftemp&&fatbuf&&fatbuf_usb)
