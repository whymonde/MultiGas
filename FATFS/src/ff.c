...
***********************************************/
/* 删除文件夹内的所有文件和文件夹 */
FRESULT FS_DeleteIntFile(char * path)
{
    UINT i, j;
    FRESULT res;
    DIR dir;
...
/* 删除文件夹或文件 */
// 注意参数是一个 RAM 指针，千万不用CONST
FRESULT FS_DeleteFolderOrFile(char * path)
{
    FRESULT res;
...
#if _USE_STRFUNC
/*-----------------------------------------------------------------------*/
/* Get a string from the file    结束后指针指到'\n'后一位                */
/*-----------------------------------------------------------------------*/
...
)
{
	...
	while (n < len - 1) {	/* Read characters until buffer gets filled */
		...
		c = s[0]; //分析读到的数据
		if (c >= 0x80) {
			if (c < 0xC0) continue;	/* Skip stray trailer   0x80---0xc0 */
			if (c < 0xE0) {			/* Two-byte sequence        0xc0---0xe0 */
				f_read(fp, s, 1, &rc);
				if (rc != 1) break;
				c = (c & 0x1F) << 6 | (s[0] & 0x3F);
				if (c < 0x80) c = '?';
		...
		if (rc != 2) break;
		c = s[0] + (s[1] << 8);
#else						/* Read a character in ANSI/OEM  读取单个字节*/
		f_read(fp, s, 1, &rc);
		if (rc != 1) break;
		c = s[0];
		if (IsDBCS1(c)) {
		...
		if (!c) c = '?';
#endif
#else						/* Read a character without conversion        现在的配置为采用以下程序*/
		f_read(fp, s, 1, &rc);                              //每读一次成功，指针移到下一位
		if (rc != 1) break;
		c = s[0];
#endif
